...
// Helper function to map ids and similarities to strings
function mapIdsToStringsAndScores(
  ids: number[],
  scores: number[],
  strings: string[]
) {
  return ids.map(
    (id, index) => `${strings[id]} (Score: ${scores[index].toFixed(2)})`
  );
}

// Get top K embeddings
const [topKSimilarities, topKIds] = getTopKEmbeddings(
  queryEmbedding,
  contextEmbeddings,
  5
);
...
console.log(
  mapIdsToStringsAndScores(topKIds, topKSimilarities, contextStringArray)
);

// Get MMR embeddings
const [mmrSimilarities, mmrIds] = getTopKMMREmbeddings(
  queryEmbedding,
  contextEmbeddings,
  5
);
...
console.log(
  mapIdsToStringsAndScores(mmrIds, mmrSimilarities, contextStringArray)
);
